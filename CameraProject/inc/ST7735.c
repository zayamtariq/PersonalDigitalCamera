...
void ST7735_DrawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  setAddrWindow(x,y,x+1,y+1); // original code 
...
void ST7735_uBinOut5(uint32_t n) { 
  ...
  if(n >= 32000){ // if input is greater than 63999    
    ST7735_OutString(" ***.**");// output "***.**"
    return;
  }else{
      if(t < 100){                 // if the number is less than 1,
...
static int32_t minX_g, maxX_g, minY_g, maxY_g;

void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY){
	ST7735_FillRect(0, 0, 128, 160, ST7735_BLACK); 
	ST7735_OutString(title); 
	ST7735_PlotClear(20, 160);
	minX_g = minX; 
	maxX_g = maxX; 
	minY_g = minY; 
	maxY_g = maxY; 
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
	int size = sizeof *bufX / sizeof bufX[0]; 
	
  for (int i = 0; i < size; i++) { 
		if (bufX[i] > minX_g && bufX[i] < maxX_g && bufY[i] > minY_g && bufY[i] < maxY_g) { 
