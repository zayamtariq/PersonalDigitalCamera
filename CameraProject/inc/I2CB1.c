...
void I2C0_Init(uint32_t I2Cfreq, uint32_t busFreq){
  ...
  GPIO_PORTB_DR8R_R |= 0x0C;            //  high current on P3,2
  GPIO_PORTB_DEN_R |= 0x0C;             // 5) enable digital I/O on PB2,3
                                        // 6) configure PB2,3 as I2C
  GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R&0xFFFF00FF)+0x00003300;
  GPIO_PORTB_AMSEL_R &= ~0x0C;          // 7) disable analog functionality on PB2,3
  I2C0_MCR_R = I2C_MCR_MFE;             // 9) master function enable, no glitch
  I2C0_MCR2_R = I2C_MCR2_GFPW_BYPASS;   // bypass glitch
...
int I2C0_Send2(int8_t slave, uint8_t data1, uint8_t data2){
  while(I2C0_MCS_R&0x01){}; // wait for I2C ready
  I2C0_MSA_R = slave<<1;    // MSA[7:1] is slave address
                            // MSA[0] is 0 for send
  I2C0_MDR_R = data1;       // prepare first byte
  I2C0_MCS_R = 0x03; // no stop, generate start, enable
  while(I2C0_MCS_R&0x01){}; // wait for transmission done
  if((I2C0_MCS_R&0x0E) != 0){ 
    I2C0_MCS_R = 0x04;      // send stop if error
    return I2C0_MCS_R&0x0E; // return error bits
  }
  I2C0_MDR_R = data2;       // prepare second byte
  I2C0_MCS_R = 0x05;        // generate stop, no start, enable
  while(I2C0_MCS_R&0x01){}; // wait for transmission done
  return I2C0_MCS_R&0x0E;   // return error bits, if any
}

//int I2C0_Send2b(int8_t slave, uint8_t data1, uint8_t data2){
//  while(I2C0_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
...
uint8_t I2C0_Recv1(int8_t slave){
  int retryCounter = 1;
  do{
    while(I2C0_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
    I2C0_MSA_R = (slave<<1)&0xFE;    // MSA[7:1] is slave address
...
uint16_t I2C0_Recv2(int8_t slave){
  uint8_t data1,data2;
  int retryCounter = 1;
  do{
    while(I2C0_MCS_R&I2C_MCS_BUSY){};// wait for I2C ready
...
int I2C0_SendData(uint8_t slaveAddr, uint8_t *pData, uint32_t count){
 ...
 // I2C0_MSA_R &= ~I2C_MSA_RS;                         // MSA[0] is 0 for send
  ...
  I2C0_MDR_R = 0x40;           // first byte 0x40 means data bytes
  I2C0_MCS_R = I2C_MCS_RUN|I2C_MCS_START;           // run and start
  while (I2C0_MCS_R & I2C_MCS_BUSY){};              // wait for transmission done
//  if((I2C0_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0){
//    I2C0_MCS_R = I2C_MCS_STOP;                      // stop transmission
...
//  }
  for(int i = 0; i < count-1; i++) {
    I2C0_MDR_R = pData[i] ;         // prepare data byte
    I2C0_MCS_R = I2C_MCS_RUN;                       // master enable
    while (I2C0_MCS_R & I2C_MCS_BUSY){};            // wait for transmission done
      // check error bits
//    if((I2C0_MCS_R & (I2C_MCS_DATACK|I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0){
...
//    }
  ...
  I2C0_MDR_R = pData[count-1] ;     // prepare last byte
  I2C0_MCS_R = (I2C_MCS_STOP |                      // generate stop
                I2C_MCS_RUN);                       // master enable
  while (I2C0_MCS_R & I2C_MCS_BUSY) {};             // wait for transmission done
  // return error bits
